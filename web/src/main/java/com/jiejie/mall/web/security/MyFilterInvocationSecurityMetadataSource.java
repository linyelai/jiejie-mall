package com.jiejie.mall.web.security;


import com.jiejie.mall.web.domain.Menu;
import com.jiejie.mall.web.domain.Role;
...
public class MyFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) {
        ...
        memberRole.setName("ROLE_member");//ROLE_member
        Menu adminmenu = new Menu();
        roles1.add(adminRole);
        adminmenu.setRoles(roles1);
        adminmenu.setUrl("/admin");
        allMenu.add(adminmenu);
        //
        Menu helloMenu = new Menu();
        helloMenu.setUrl("/hello");
        roles2.add(memberRole);
        roles2.add(adminRole);
      //  roles1.add(member);
        helloMenu.setRoles(roles2);
        allMenu.add(helloMenu);
        for (Menu menu : allMenu) {
            if (antPathMatcher.match(menu.getUrl(), requestUri)
                ...
                log.info("当前访问路径是{},这个url所需要的访问权限是{}", requestUri, values);
         ...
         **/
        log.info("当前访问路径是{},这个url所需要的访问权限是{}", requestUri, "ROLE_LOGIN");
